    ...
    <section id="about" className="px-5 py-24 md:px-8 md:py-32">
      <div className="mx-auto grid max-w-6xl grid-cols-1 items-start gap-12 md:grid-cols-2 md:gap-16 lg:gap-20">
        <Reveal direction="left" className="group">
          <Parallax speed={0.22}>
            <div className="relative aspect-[3/4] w-full max-w-md overflow-hidden rounded-2xl bg-[#161616] transition-shadow duration-500 hover:shadow-2xl hover:shadow-black/50 md:max-w-none">
              <img
        ...
        <Reveal direction="right" delay={120}>
          <div className="flex flex-col justify-center">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#FF5733] transition-colors duration-300 hover:text-[#ff7a5c]">
              a bit about myself
            </p>
            ...
            <ul className="mt-10 flex flex-wrap gap-2">
              {skillTags.map((tag) => (
                <li key={tag}>
                  <span className="hover-pill inline-block cursor-default rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium text-[#c4c4cc]">
                    {tag}
                  </span>
                </li>
              ))}
